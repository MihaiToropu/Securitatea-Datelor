...
char BeaufortEncrypt(char *alphabet, int *intKey, char text, int position)
{
    int encryptedText;
...
char BeaufortDecrypt(char *alphabet, int *intKey, char text, int position)
{
    int decryptedText;
...
int main() {
    ...
    switch (x)
    {
        case 1:
            ...
            {
                encryptedText[i] = BeaufortEncrypt(alphabet, intKey, plainText[i], position % keyLength);
                ++position;
            }
        ...
        case 2:
            for(int i = 0; i < textLength; i++)
            {
                decryptedText[i] = BeaufortDecrypt(alphabet, intKey, plainText[i], position % keyLength);
                ++position;
            }
